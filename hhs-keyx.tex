...
its codomain $E/E[\frak a]$ is well-defined up to isomorphism
...
This choice is equivalent to fixing the Frobenius as an element in $\O$,
...
Algorithm~\ref{alg:isogeny-KeyGen}
implements key generation for the above scheme.
...
prescribed order \emph{modulo small primes}. 
...
then, this ideal evaluation step could possibly performed in polynomial
...
Unfortunately, the analysis of this algorithm is only asymptotic, 
and considers particularly special $N$; it cannot be directly used
to draw conclusions on concrete parameters at this stage.
...
algorithm described in~\cite[§2.1]{regev04} in the case $N = 2^n$ 
requires about $2^{4\sqrt{\log N}}$ black-box queries.
Therefore, choosing this estimate
is a reasonable choice to propose concrete parameter sizes
(we mention that Kuperberg's original analysis only gave 
a complexity of $2^{3\sqrt{\log N}}$ operations).
Of course, this assumption is only heuristic, and should be
validated by further study of quantum dHSP solvers; at present
time, unfortunately, no precise statement can be made.

Table~\ref{tab:sizes} thus proposes three parameter sizes,
...
\begin{table}
    ...
    \begin{tabular}{c@{\;}|@{\;}c@{\;}|@{\;}c@{\;}|@{\;}c@{\;}|@{\;}c}
        ...
        $768$  & $384$ & $2^{192}$ & $> 2^{78}$ & 1\\
        $1024$ & $512$ & $2^{256}$ & $> 2^{90}$ & 1
        \\
        \hline
    \end{tabular}
    \smallskip
...
programming language. This proof of concept also allows
us to estimate the cost of isogeny steps, which is needed
in the process of generating the initial curve in Section~\ref{sec:initcurve}.
