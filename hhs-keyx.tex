...
we will use in the sequel. We will denote by $\Card(S)$ the cardinality of
a finite set $S$; and if $E$ is an elliptic curve (or a subgroup of an
elliptic curve), and $k$ is a field, we will denote by $E(k)$ the set of
points on $E$ that are defined over $k$. Of course, this makes sense only
when $k$ is an extension of the field of definition of $E$.
