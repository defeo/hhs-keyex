...
\begin{abstract}
    ...
    for contemporary pre- and post-quantum security.
...
we can also avoid computing $y$-coordinates in Algorithm~\ref{alg:ElkiesStep}
at line~\ref{line:direction} if $\lambda\neq\pm\mu$. In this case,
choosing the correct quadratic twist at line~\ref{line:end_direction}
is not an issue.
...
to be used in Algorithm~\ref{alg:keygen}. Table~\ref{tab:VéluSteps}
lists the isogeny degrees amenable to Algorithm~\algstyle{VéluStep}, with
the corresponding extension degree $r$ (a star denotes that the twisted curve
