...
The origins of isogeny-based cryptography can be traced back to
...
primitive became even more unusable in a quantum
security setting.
...
Although the final result is far from being competitive, we believe it
...
We take a generating set consisting of ideals over a set 
$S$ of small Elkies primes,
which we partition into three sets according to the step algorithms
...
Unfortunately, and in contrast with the case of supersingular curves, 
...
lower bound,
%, given that the described algorithm is specific to $h(O_K)=2^n$, and also 
...
%By taking into account the fact that each
...
\begin{definition}[Isogeny Walk DDH (IW-DDH)]
    ...
    %one of the following two distributions
%    \begin{align*}
...
\begin{definition}[Isogeny Walk Distinguishing (IWD)]
    Given a triplet of curves $(E_a,E_b,E_{ab})$
    sampled with probability $\frac{1}{2}$ 
    from $\mathcal{G}_{q,Δ}$
    and $\frac{1}{2}$ from $\mathcal{W}_{q,Δ,σ}$,
    decide from which it was sampled.
...
\begin{definition}[Class Group Action DDH (CGA-DDH)]
    Given a triplet of curves $(E_a,E_b,E_{ab})$
    sampled with probability $\frac{1}{2}$ 
    from $\mathcal{U}_{q,Δ}$
    and $\frac{1}{2}$ from $\mathcal{G}_{q,Δ}$,
    decide from which it was sampled.
%    from one of the following two distributions
%    \begin{align*}
