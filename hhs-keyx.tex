...
By definition, an isogeny $ϕ:E→E'$ is
...
exchange protocol as in Algorithm~\ref{proto:cayley}. The
commutativity of $G$ is essential to ensure Alice and Bob
...
Remember that an isomorphism class of elliptic curves determines
a unique $j$-invariant in $\F_q$. While is is more convenient to
compute isogeny steps using ``real'' curves, all communications
between Alice and Bob should only mention $j$-invariants.

\subsection{Sampling and evaluating isogeny walks}

From a computational point of view, we now face the following
question: given $g\in\Cl(\O)$ given as a product of small ideals
and an elliptic curve $E$, compute the action $g\cdot E$.
The cost of this operation is the sum of the costs of each
isogeny step involved, \emph{which depends on the ideal}.
We will thus minimize the global
computational cost by means of adapted, or \emph{skewed},
smooth representations for elements in $\Cl(\O)$.
...
\begin{itemize}
\item Algorithm~\algstyle{ElkiesStep}$(E, \ell, \lambda)$ (Algorithm~\ref{alg:ElkiesStep}) that uses
	the \emph{modular polynomial} $\Phi_\ell(X, Y)$, and works with any $\frak s$;
\item Algorithm~\algstyle{VéluStep}$(E, \ell,\lambda)$ (Algorithm~\ref{alg:VéluStep}) that
...
The introduction of Algorithm~\algstyle{VéluStep}, very different in
spirit from Algorithm~\algstyle{ElkiesStep} and
inspired by the SIDH cryptosystem and related protocols,
speeds up the protocol by a considerable factor and
is the main contribution of our work. Vélu steps are also
a key ingredient in the CSIDH protocol.
Let us now delve into
the details of these two~\algstyle{Step} algorithms.

\begin{algorithm}
	\caption{\algstyle{ElkiesStep}}
	\label{alg:ElkiesStep}
	\Input{$E\in\Ell_q(\O)$, $(\ell,\lambda)$ defining an ideal $\frak s = (\pi-\lambda,\ell)$}
	\Output{$\frak s\cdot E$}
	$P\gets \Phi_\ell(X, j(E))$ \;
	$j_1, j_2 \gets$ Roots of $P$ over $\F_q$ \label{line:roots} \;
	$K\gets$ Kernel of the $\ell$-isogeny from $E$ to some $E_1$ with $j(E_1) = j_1$ \label{line:kernel}\;
	$Q\gets$ a nonzero point in $K$\;
	\lIf{$\pi(Q) = [\lambda]Q$}{$j_R\gets j_1$} \label{line:direction}
	\lElse{$j_R\gets j_2$}
	$E_R \gets$ a curve verifying $j(E_R) = j_R$ \label{line:end_direction} \;
	\Return{$E_R$}
\end{algorithm}
...
\begin{algorithm}
	...
	\Input{$E\in\Ell_q(\O)$, $(\ell,\lambda)$ defining an ideal $\frak s = (\pi-\lambda,\ell)$}
	\Output{$\frak s\cdot E$}
	$r\gets$ Multiplicative order of $\lambda$ in $\Z/\ell\Z$ \;
	$C_r \gets \# E(\F_{q^r})$ \label{line:card} \;
	\Repeat{$Q \neq 0_E$}{
		$P\gets$ Random point in $E(\F_{q^r})$ \;
		$Q\gets [C_r/\ell] P$ \label{line:scalarmult} \;
	}
	$K\gets$ Subgroup of order $\ell$ generated by $Q$ \;
	$E_R\gets E/K$ \label{line:quotient} \;
	\Return{$E_R$}
\end{algorithm}

\paragraph{Elkies steps.}
The validity of Algorithm~\ref{alg:ElkiesStep} is an immediate
consequence of the following property of the modular polynomial $\Phi_\ell$:
...
The second part of the algorithm (lines~\ref{line:kernel}
to~\ref{line:end_direction}) determines which $j$-invariant
between $j_1$ and $j_2$ gives the correct direction in the isogeny
graph (recall Figure~\ref{fig:cycle}), and should only be done
for the \emph{first} action of the ideal $\frak s$, since no return
on the isogeny circle is made. The kernel computation at line~\ref{line:kernel}
is a classical operation about isogenies; for instance, one can
use the Bostan--Morain--Salvy--Schost algorithm~\cite{BMSS08}, which
costs $\softO(\ell)$ $\F_q$-operations using asymptotically fast
...
Thus, the cost of Algorithm~\algstyle{ElkiesStep} is $\softO(\ell\log q)$
field operations, and grows quasi-linearly with $\ell$.
...
using Vélu's formulas~\cite{Velu71} using $O(\ell)$ operations in $\F_q$.
...
To summarize:
\begin{itemize}
\item Algorithm~\algstyle{ElkiesStep} costs $\softO(\ell\log q)$ operations in $\F_q$;
\item Algorithm~\algstyle{VéluStep} costs $\softO(r^2\log q)$ operations in $\F_q$,
where $r$ is the multiplicative order of $\lambda$ in $\Z/\ell\Z$.
\end{itemize}
In the generic case $r = O(\ell)$, so Algorithm~\algstyle{ElkiesStep}
should be preferred. However, when $r$ is small,
a factor $\ell$ is saved using Algorithm~\algstyle{VéluStep}.
...
satisfies congruence modulo small primes.
...
easily described (Algorithm~\ref{alg:keygen}).

\begin{algorithm}
\caption{\algstyle{KeyGen}}
\label{alg:keygen}
\Input{The set $S$ given as $S_E, S_{V,2}, S_{V,1}$; the bounds $M_\ell$}
\Output{A secret exponent vector $(k_\ell)$}
\For{$(\ell, \lambda, \mu)\in S_E\cup S_{V, 2}$}
{
	$k_\ell\uni [-M_\ell, M_\ell]$ \;
}
...
see Algorithm~\ref{alg:walk}. Algorithm~\ref{alg:keyex}
then rephrases the generic Diffie--Hellman algorithm
...
\begin{algorithm}
  ...
  \Alice{
    ...
    \Sends $j_A = \algstyle{Walk}(k_A, j(E_0), S)$ to Bob}
  \Bob{
    \Samples $k_B = \algstyle{KeySample}(S, (M_\ell))$\;
    \Sends $j_B = \algstyle{Walk}(k_B, j(E_0), S)$ to Alice}
  \Key{$j_F = \algstyle{Walk}(k_A, j_B, S) = \algstyle{Walk}(k_B, j_A, S)$}
\end{algorithm}
...
\item The keyspace size is at least $\sqrt{|Δ_K|}$.
\end{itemize}
...
Nemo~\cite{nemo}. For this, we developed a small
...
Using Algorithm~\ref{alg:ElkiesStep}, we approximate the
time to compute a step for $\frak s = (\pi-\lambda,\ell)$
as the cost to find the roots of the modular polynomial,
which is roughly $0.017\cdot\ell$ seconds in our implementation.
Using Algorithm~\ref{alg:VéluStep}, the cost of a step is approximated
as that of the scalar multiplication in $E(\F_{q^r})$ as follows:
...
that the \emph{total time spend on $\ell$-isogenies} is less than $T$.
...
%todo: put these two tables on the same page ?
\begin{table}
\centering
\begin{tabular}{cclc}
$r$ & $M_\ell$ & Primes \\
...
9 & 6 & 1693
\end{tabular}
    \caption{Primes $\ell$ for which Algorithm~\algstyle{VéluStep} is
...
1 &  241,
	...
	\\ &
     ...
     467 \\
\end{tabular}
    \caption{Primes $\ell$ for which Algorithm~\algstyle{ElkiesStep} is
