...
\institute{
 ...
 École Normale Supérieure (Paris), Inria Saclay (Palaiseau),\\
 Université de Bordeaux (Bordeaux), France\\
 \url{jean.kieffer@ens.fr} \and
...
also more practical. While being very slow, it does not suffer from potential cryptographic weaknesses SIDH has (such as short paths and the publication of image points), and permits static-static protocols.
...
This paper aims to improve and modernize the
...
In Section~\ref{sec:exp} we present a
proof-of-concept implementation and measure its performance.
The final result is far from being competitive, but we believe it
...
Sections~\ref{sec:keyex}--\ref{sec:initcurve}, using them in a
...
Section~\ref{sec:keyex}. When $Δ_π$ is square-free, $ℤ[π]$
...
$
(according to Section~\ref{sec:sec}), using binary search.
While the $M_\ell$
