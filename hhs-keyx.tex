...
$K=ℚ(\sqrt{Δ_π})$ (see~\cite[III.9]{silverman:elliptic}).  Denote by
$\O_K$ the ring of integers of $K$, it is its \emph{maximal order},
i.e.\ it contains any other order of $K$.  Hence $ℤ[π]⊂\End(E)⊂\O_K$,
and there is only a finite number of possible orders for $\End(E)$:
write $Δ_π=d^2Δ_K$, where $Δ_K$ is the discriminant of $\O_K$, then
the index $[\O_K:\End(E)]$ must divide $d=[\O_K:ℤ[π]]$.
...
$d=\sqrt{Δ_π/Δ_K}$, and thus correspond to cases (1.x) in the previous
subsection.
% todo: do we frown upon bold emphasis?
