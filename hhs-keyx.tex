...
Following Couveignes~\cite{cryptoeprint:2006:291} and
Rostovtsev and Stolbunov~\cite{rostovtsev+stolbunov06},
...
The solution is to fix a subset of small prime elements \(S\subset G = \Cl(\O)\) 
whose action on \(X = \Ell_q(\O)\) can be computed efficiently,
and such that compositions of (powers of) elements of \(S\)
...
as a walk in the Cayley graph of \(G\) generated by \(S\),
sometimes called a \emph{Schreier graph}.
Given a subset \(S\) of a group \(G\)
and a principal homogeneous space \(X\) for \(G\),
we define the Schreier graph $\Graph(G, S, X)$
to be the directed graph whose vertices are the elements of $X$, 
and where an edge labelled by $s∈S$
links $x_1$ to $x_2$ if and only if $s\cdot x_1 = x_2$.
If $S$ is symmetric (i.e.\ $S^{-1}=S$), 
then we associate the same label to $s$ and $s^{-1}$, 
and make the graph an undirected one. 
...
$(s_i)_{1≤i≤n}∈S^*$ of \emph{steps} in $S$. 
...
\item Return the exponent vector $(k_\ell)_\ell$.
\end{itemize}
...
curves.  
Specifically, we use the \emph{classical modular polynomial} $\Phi_\ell(X, Y)$.
