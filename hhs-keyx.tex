...
\begin{itemize}
    \item Algorithms~\ref{alg:ElkiesFirstStep}
        (\algstyle{ElkiesFirstStep}, for the first application of $\frak{s}$)
        and~\ref{alg:ElkiesNextStep} 
        (\algstyle{ElkiesNextStep}, for higher powers of $\frak{s}$)
        which use the \emph{modular polynomial} $\Phi_\ell(X, Y)$, 
        and works for any $\frak s$;
...
\begin{algorithm}
	...
	\Input{%
        $E\in\Ell_q(\O)$; 
        $(\ell,\lambda)$ defining $\frak s = (\pi-\lambda,\ell) \subset \O$
    }
...
\begin{algorithm}
    ...
    \Input{%
        $(\ell,\lambda)$ defining $\frak{s} = (\ell,\pi-\lambda)\subset\O$;
