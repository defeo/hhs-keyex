...
a key ingredient in the CSIDH protocol~\cite{csidh}.
...
\begin{algorithm}
	...
	\lElse{$j_R\gets j_2$} \label{line:end_direction}
	\Return{$j_R$}
\end{algorithm}
...
\begin{algorithm}
	...
	\Input{$(\ell,\lambda)$ defining an ideal $\frak s = (\pi-\lambda,\ell)$;
		$j_{k-1} = j(\frak s^{k-1}\cdot E)$ and $j_k = j(\frak s^{k}\cdot E)$ for some $E\in\Ell_q(\O)$ and $k\geq 1$}
	\Output{$j(\frak s^{k+1}\cdot E)$}
	$P\gets \Phi_\ell(X, j_k) / (X - j_{k-1})$ \;
	$j_{k+1}\gets$ the unique root of $P$ in $\F_q$ \;
	\Return{$j_{k+1}$}
\end{algorithm}
...
\begin{algorithm}
	...
	\Input{$E\in \Ell_q(\O)$ with order $C$; $(\ell,\lambda)$ defining an ideal $\frak s = (\pi - \lambda,\ell)$; $k\geq 1$}
	\Output{$\frak s^k \cdot E$}
	$j_0 \gets j(E)$ \;
	$j_1 \gets \algstyle{ElkiesStep}(E, \ell, \lambda)$ \;
	\For{$2\leq i \leq k$}
	{
		$j_0, j_1 \gets j_1, \algstyle{ElkiesNextStep}(\ell, \lambda, j_0, j_1)$ \;
	}
	$E_R \gets$ an elliptic curve with $j$-invariant $j_1$ and order $C$ \label{line:twist} \;
	\Return{$E_R$}
\end{algorithm}
...
The kernel computation at line~\ref{line:kernel} in Algorithm~\ref{alg:ElkiesStep}
...
using a Cantor--Zassenhaus-type algorithm. This is also the critical
part of Algorithm~\ref{alg:ElkiesNextStep}.
Thus, the cost of Algorithms~\algstyle{ElkiesStep}
...
remembering the previous curve. Algorithm \algstyle{ElkiesWalk}
(Algorithm~\ref{alg:ElkiesWalk}) shows how to combine these two
...
its quadratic twist has).
Remark that Algorithm~\ref{alg:ElkiesNextStep} still has to compute the
...
eigenvalue, must not divide $r$. Notice that the values
...
\label{alg:keygen}
\Input{The set $S$ given as $S_E, S_{V,2}, S_{V,1}$; the walk length bounds $M_\ell$}
