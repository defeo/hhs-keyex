...
\begin{algorithm}
    ...
    \For{$(\ell, \lambda, \nu) \in S_{VV}$}{%
        ...
        $E \gets \algstyle{VeluWalk}(E,(\ell,\nu),|k_\ell|)$
        \;
    }
    \For{$(\ell, \lambda)\in S_{VE}$}{%
        $k_\ell\gets \algstyle{Random}([0,M_\ell])$ 
        \;
        $E \gets \algstyle{VeluWalk}(E,(\ell,\lambda),k_\ell)$
        \;
    }
...
the constants implied in the complexity are not made explicit, 
so it is very hard to estimate the impact of this attack
...
Therefore, we choosing this estimate
