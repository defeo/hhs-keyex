...
remark that the weaker version of Kuperberg's algorithm, as described
...
storage, is a much more realistic candidate to run on a real world
quantum computer. By taking into account the fact that each black box
query runs a quantum circuit of subexponential size itself, we
estimate that the lower bound on the number of quantum gates is at
...
\begin{table}
  ...
  \begin{tabular}{c | c | c | c | c}
    $\log Δ_π$ & $\log h(\O_k)$ & classical security & quantum queries & NIST category\\
    \hline
    $512$  & $256$ & $2^{128}$ & $> 2^{64}$ & 2\\
    $768$  & $284$ & $2^{192}$ & $> 2^{78}$ & 3\\
    $1024$ & $512$ & $2^{256}$ & $> 2^{90}$ & 5
  \end{tabular}
