...
According to section~\ref{todo}, the global performances of our protocol
...
small primes. Even without using algorithm~\ref{alg:step5}, the scheme is
...
$C = \Card E(\F_p)$, it computes the value of $C\mod\ell$ modulo a series
of small primes $\ell$: it is a \emph{multimodular algorithm}.
Usually, cryptographers are interested in generating elliptic curves of
...
$C_0 = \Card E_0(\F_p)$ satisfies $C=0\mod\ell$.

We now proceed as follows:
\begin{itemize}
\item Choose a smoothness bound $B$ (we used $B = 13$).
\item Choose size bounds $L_1$ and $L_2$: primes $\ell\leq L_1$
will be considered in algorithm \algstyle{Step2},
while only primes $\ell\leq L_2$ will be considered in algorithm
\algstyle{Step} (one could use $L_1 = 2000$ and $L_2 = 300$).
\item Pick elliptic curves at random in $\F_p$, and use the SEA
algorithm, aborting when any $\ell\leq B$ with $C\mod\ell\neq 0$
is found.
\item Among the curves which passed the tests, compute a \emph{score}
taking into account all other primes $\ell\leq L$ for which
$C = 0\mod\ell$, and the number of Elkies primes below $L_2$.
\end{itemize}
The highest-scored curves are now good candidates for the curve $E_0$.
...
elliptic curve~\cite{todo:refMontgomery}.
...
\end{aligned}
\]
It admits $\ell$-torsion rational points for each
